...
- `hot_task.inputs` schema now requires each input item to be either a non-empty string or a
  structured object with `kind` + `id` (optional `uri`, `notes`) in
  [`schema/rpo.v1.schema.json`](./schema/rpo.v1.schema.json) and
  [`src/rpo/data/rpo.v1.schema.json`](./src/rpo/data/rpo.v1.schema.json).
- UI renderer now serializes structured input items as sorted JSON for deterministic output in
...
- Renderer regression tests now assert deterministic structured-input rendering and schema
  rejection for malformed input objects in [`tests/test_render_ui.py`](./tests/test_render_ui.py)
